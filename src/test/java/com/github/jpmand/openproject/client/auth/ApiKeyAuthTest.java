...
class ApiKeyAuthTest {
    ...
    @Test
    void testApiKeyAuthAddsBasicAuthHeader() throws Exception {
    ...
    @Test
    void testApiKeyAuthWrongApiKey() throws Exception {
        String expectedCredentials = Credentials.basic("apikey", "test-api-key");

        ApiKeyAuth auth = new ApiKeyAuth("wrong-api-key");
        OpenProjectClient client = new OpenProjectClient(
                mockServer.url("/").toString(),
                auth
        );
        ...
        assertNotNull(authHeader, "Authorization header should be present");
        assertTrue(authHeader.startsWith("Basic "), "Should use Basic authentication");
        assertNotEquals(expectedCredentials, authHeader);
    }

    @Test
    void testApiKeyAuthWithEmptyKey() throws Exception {
